...
> v2.3.0 — **fully updated for Google's "Gemini Notebook" rebrand of NotebookLM.** Create / list / rename / delete notebooks, add sources, citation-backed Q&A, and every Studio generation type (audio · video · infographic · report · presentation · data table) plus listing and download were re-verified end-to-end against the rebuilt UI. Production-grade, batch-tested on overnight runs of 1 000+ questions. Tools are a namespaced tree (`notebook_ask`, `source_add`, `session_list`…) with MCP `annotations` + `outputSchema` — **the old flat names still work as aliases, nothing breaks**. See the [changelog](./CHANGELOG.md). [Compare with `PleasePrompto/notebooklm-mcp`](https://roomi-fields.github.io/notebooklm-mcp/compare) for when this project is the right pick (REST API, full Studio, auto-reauth).
